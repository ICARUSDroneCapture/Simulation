...
% Setting noise to 0 to isolate bias
accel_noise_std = 0;
gyro_noise_std = 0;
...
% Setting environmental temperature change to max possible (worst case)
...
a.biasedAccelCurve = @(t, real_accel, biasStabDistAccel, biasTempDistAccel) real_accel(t) + a.biasStabDistAccel(t) + a.biasTempDistAccel(t);
a.biasedGyroCurve = @(t, real_ang_rate, biasStabDistGyro, biasTempDistGyro) real_ang_rate(t) + a.biasStabDistGyro(t) + a.biasTempDistGyro(t);

test7 = a.biasedAccelCurve(t, a.real_accel, a.biasStabDistAccel, a.biasTempDistAccel);
test8 = a.biasedGyroCurve(t, real_ang_rate, a.biasStabDistGyro, a.biasTempDistGyro);

a.theta_err = @(t) a.biasStabDistGyro(t).*t + ARW.*sqrt(t);

a.accel_drift_vert = @(t, real_accel, theta_err, biasStabDistAccel, biasTempDistAccel) (1 + k)*real_accel(t) + biasStabDistAccel(t)  + biasTempDistAccel(t) + g*(1-cos(theta_err(t)));
a.accel_drift_horz = @(t, real_accel, theta_err, biasStabDistAccel, biasTempDistAccel) (1 + k)*real_accel(t) + biasStabDistAccel(t)  + biasTempDistAccel(t)  + g*sin(theta_err(t));
...
accel_turn_on_bias_offset = normrnd(0, accel_noise_std)+0.01;
gyro_turn_on_bias_offset = normrnd(0, gyro_noise_std)+0.01;

% Setting turnon bias to 0 to isolate bias instability;
accel_turn_on_bias_offset = 0;
gyro_turn_on_bias_offset = 0;
...
control_dynamics = @(t, state) rigidArmControl_FixedInt(t, a, state);

[t_error, sol_error]= rk4_solver(control_dynamics, tspan, s0, dt);
...
pos_err = abs(sol_error(:, 1) - sol_control(:, 1));
