function animate3D_test(t_vec,y,d,theta_D)
    ...
    %link 1
    x1 = xB + platform.l1*cos(d2).*cos(d3).*cos(y(3,:)).*cos(y(5,:)) - platform.l1*cos(y(3,:)).*sin(y(5,:)).*(cos(d1).*sin(d3) - cos(d3).*sin(d1).*sin(d2)) - platform.l1*sin(y(3,:)).*(sin(d1).*sin(d3) + cos(d1).*cos(d3).*sin(d2));
    y1 = yB + platform.l1*sin(y(3,:)).*(cos(d3).*sin(d1) - cos(d1).*sin(d2).*sin(d3)) + platform.l1*cos(y(3,:)).*sin(y(5,:)).*(cos(d1).*cos(d3) + sin(d1).*sin(d2).*sin(d3)) + platform.l1*cos(d2).*cos(y(3,:)).*cos(y(5,:)).*sin(d3);
    z1 = zB + platform.l1*cos(d2).*cos(y(3,:)).*sin(d1).*sin(y(5,:)) - platform.l1*cos(y(3,:)).*cos(y(5,:)).*sin(d2) - platform.l1*cos(d1).*cos(d2).*sin(y(3,:));

    %link 2
    x2 = xB + cos(d2).*cos(d3).*(platform.l1*cos(y(3,:)).*cos(y(5,:)) + platform.l2*cos(y(5,:)).*cos(y(3,:) + y(1,:))) - (sin(d1).*sin(d3) + cos(d1).*cos(d3).*sin(d2)).*(platform.l1*sin(y(3,:)) + platform.l2*sin(y(3,:) + y(1,:))) - (cos(d1).*sin(d3) - cos(d3).*sin(d1).*sin(d2)).*(platform.l1*cos(y(3,:)).*sin(y(5,:)) + platform.l2*sin(y(5,:)).*cos(y(3,:) + y(1,:)));
    y2 = yB + (cos(d1).*cos(d3) + sin(d1).*sin(d2).*sin(d3)).*(platform.l1*cos(y(3,:)).*sin(y(5,:)) + platform.l2*sin(y(5,:)).*cos(y(3,:) + y(1,:))) + (cos(d3).*sin(d1) - cos(d1).*sin(d2).*sin(d3)).*(platform.l1*sin(y(3,:)) + platform.l2*sin(y(3,:) + y(1,:))) + cos(d2).*sin(d3).*(platform.l1*cos(y(3,:)).*cos(y(5,:)) + platform.l2*cos(y(5,:)).*cos(y(3,:) + y(1,:)));
    z2 = zB + cos(d2).*sin(d1).*(platform.l1*cos(y(3,:)).*sin(y(5,:)) + platform.l2*sin(y(5,:)).*cos(y(3,:) + y(1,:))) - sin(d2).*(platform.l1*cos(y(3,:)).*cos(y(5,:)) + platform.l2*cos(y(5,:)).*cos(y(3,:) + y(1,:))) - cos(d1).*cos(d2).*(platform.l1*sin(y(3,:)) + platform.l2*sin(y(3,:) + y(1,:))); 
