...
%% To Demo the ODE-induced walk-off (uncomment the following)

alpha = 0.42; % wave amplitdue [m]
...
fprintf('\n\nBeginning ODE integration (without sensor error).\n\n');
...
[t_ode_control, s_ode_control] = ode23s(@(t,s)noError_ODEFunc(t,s,a),tspan,prev_state,op);

% Checking that both simulations finished to desired time
if t_ode_control(end) ~= finishTime
    fprintf('\n\nODE Integration (without sensor error) failed to finish.\nBeginning ODE integration (with sensor error).\n\n');
else
    fprintf('\n\nODE Integration (without sensor error) finished.\nBeginning ODE integration (with sensor error).\n\n');
end
...
legend('Integration (without sensor error) with ODE')
...
% Running Simulation
op = odeset('RelTol',tolerance,'AbsTol',tolerance); % Tolerance options
[t_ode_error, s_ode_error] = ode23s(@(t,s)rigidArmControl_ODEFunc(t,s,a),tspan,prev_state,op);

% Checking that both simulations finished to desired time
if t_ode_error(end) ~= finishTime
    fprintf('\n\nODE Integration (with sensor error) failed to finish.\nBeginning fixed integration (without sensor error).\n\n');
else
    fprintf('\n\nODE Integration (with sensor error) finished.\nBeginning fixed integration (without sensor error).\n\n');
end

figure;
plot(t_ode_control, s_ode_control(:,1))
hold on
plot(t_ode_error, s_ode_error(:,1))
title('Platform Inertial Position vs Time')
xlabel('Time (s)')
ylabel('Position (m)')
title('Platform Inertial Position over Time')
legend('Integration (without sensor error) with ODE', 'Integration (with sensor error) with ODE')
...
sz = 2;
figure
scatter(1:length(t_ode_error), t_ode_error, sz, 'filled', displayName="With Error")
hold on
scatter(1:length(t_ode_control), t_ode_control, sz, 'filled', displayName="Without Error")
ylabel('Time Values')
...
if size(t_ode_control) > size(t_ode_error)
    t_precise = t_ode_control;
    p_precise = s_ode_control(:,1);
    t_compare = t_ode_error;
    p_compare = s_fix_error(:,1);
else
    t_precise = t_ode_error;
    p_precise = s_ode_error(:,1);
    t_compare = t_ode_control;
    p_compare = s_ode_control(:,1);
end
...
ylabel('Position (m)')
title('Platform Inertial Position over Time')
legend('Integration (without sensor error) with ODE', 'Integration (with sensor error) with ODE', 'Fixed-Step (without sensor error) Integration')
...
ylabel('Position (m)')
title('Platform Inertial Position over Time')
legend('Integration (without sensor error) with ODE', 'Integration (with sensor error) with ODE', 'Fixed-Step (without sensor error) Integration', 'Fixed-Step (with sensor error) Integration')
