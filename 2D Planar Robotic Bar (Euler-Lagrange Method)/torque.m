function torque = torque(time, y, r_0, ref_q, r_0_ref,p_ddot, angle2, gains)
%SUMMARY
...
diff_q1Dif = diff(ref_q-q1(t),'t');

% writting int(ref_q1-q1(t)) as a vector
int_q1Dif = cumtrapz(time,ref_q-y(1,:));
...
for i=1:length(time)
    ...
    torque(:,i) = Kp1*(ref_q-y(1,i))+Ki1*int_q1Dif(i)+Kd1*diff_q1Dif_eval+(J_eval')*F_control_eval;
end
