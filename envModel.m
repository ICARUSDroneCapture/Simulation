...
%% Trochoidal Environment

% Equations for reference: https://en.wikipedia.org/wiki/Trochoidal_wave
% Reference for script: https://stackoverflow.com/questions/28279337/matlab-cuda-ocean-wave-simulation


% Credit: https://github.com/Hokitlab/OceanSimulator/blob/master/ocean_simulator.m
...
centerLoc = size(Z)/2;
centerPoint = Z(centerLoc(1), centerLoc(2));

%// Display the initial wave surface
set( h.center , 'XLoc', centerLoc(1), 'YLoc', centerLoc(2), 'ZLoc', centerPoint)
...
function animate_wave(h,~)
    persistent time H0 W Grid_Sign hs hc
    ...
    Z = calc_wave( H0,W,time,Grid_Sign ) ;
    center = size(Z)/2;
    centerPoint = Z(center(1), center(2));
    hs.ZData = Z ;
    hc.ZLoc = centerPoint;
...
h.center = NaN ;                        %// create an empty "center" object
