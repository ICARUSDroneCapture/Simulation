...
% Rows represents timesteps, max limit of 5 timesteps kept track of
integrator_type = 2;
state_record_temp = zeros(5, 2);
deriv_record_temp = zeros(5, 2);
state_int_temp = zeros(length(t), 2);

for i = 1:length(t)

    time = t(i)

    val_dot_temp = [a.biasTempDistAccel(t) a.biasTempDistGyro(t)];
    deriv_record_temp = insertVector(deriv_record_temp, val_dot_temp);

    state_vec_temp = fdm_integrator(state_record_temp, deriv_record_temp, dt, integrator_type);

    state_record_temp = insertVector(state_record_temp, state_vec_temp);

    state_int_temp(i, :) = state_vec_temp;
end

vel_temp = state_int_temp(:, 1);
angle_temp = state_int_temp(:, 2);

figure
subplot(1,2,1)
plot(t, vel_temp);
xlabel('Time (s)')
ylabel('Noise Offset (m/s)')
title('Accelerometer Temp Bias Walk')
subplot(1,2,2)
plot(t, angle_temp);
xlabel('Time (s)')
ylabel('Noise Offset (deg)')
title('Gyroscope Temp Bias Walk')
